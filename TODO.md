...
| R1 | Dead UI code found during the help audit. `TfrmEditAuthorDataEx` (`frm_EditAuthorEx.pas`) is never instantiated — the extended author editor is unreachable; decide whether to remove it or wire it up. Also `acHelpSupportForum` (`frm_main.dfm`) is defined but bound to no menu/toolbar element. | 🟡 M | 🔴 Open |
